...
#SBATCH --output=/fred/oz480/mcoughli/AppleCider/logs/rtf_wsyn_%j.out
#SBATCH --error=/fred/oz480/mcoughli/AppleCider/logs/rtf_wsyn_%j.err

set -euo pipefail

echo "=== RTF: Training with survey-sim synthetic data ==="
...
SYNTH=/fred/oz480/mcoughli/AppleCider/rtf/data_surveysim
...
# Train with real + synthetic data, 6 classes (5 real + "other" for KN/afterglow)
echo ""
echo "========== AE + meta + GP + synthetic (6 classes) =========="
python train.py \
    --data-dir $DATA \
    --synthetic-dir $SYNTH \
    --output-dir $OUTDIR \
...
python eval_early.py \
    ...
    --data-dir $DATA \
    --output-dir $ANALYSIS/early_with_synthetic \
    --num-classes 6 \
