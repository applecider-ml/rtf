...
def train_one_epoch(model, loader, optimizer, epoch, total_epochs, target_beta, device, grad_clip=1.0):
    model.train()
    beta = beta_schedule(epoch, total_epochs, target_beta) if model.mode == "vae" else 0.0
...
def train_model(
    mode, latent_dim, data_dir, output_dir,
    ...
    for epoch in range(epochs):
        t0 = time.time()
        train_m = train_one_epoch(model, train_loader, optimizer, epoch, epochs, target_beta, device)
    ...
    print(f"  Test recon: cont={test_m['recon_cont']:.4f} band={test_m['recon_band']:.4f}")
...
def main():
    ...
    for latent_dim in args.latent_dims:
        s = train_model(
            mode=args.mode, latent_dim=latent_dim,
    ...
    print(f"{'='*80}")
    print(f"{'dim':>5} {'recon_cont':>10} {'recon_band':>10} {'band_acc':>9} {'compress':>8} {'bytes':>6}")
    print("-" * 55)
    for s in all_summaries:
        tm = s["test_metrics"]
        print(f"{s['latent_dim']:5d} {tm['recon_cont']:10.4f} {tm['recon_band']:10.4f} "
